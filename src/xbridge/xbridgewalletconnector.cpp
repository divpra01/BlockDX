...
bool WalletConnector::getUtxoEntriesForAmount(const uint64_t& amount, const std::string& address, std::vector<wallet::UtxoEntry>& entries) const
{
    ...
    getUnspent(outputs);
    // Filter utxo's by address
    outputs.erase(std::remove_if(outputs.begin(), outputs.end(),
                           [address](const wallet::UtxoEntry &e) { return e.address != address; }), outputs.end());
